...
Демо-аккаунты (создаются при первом старте, все данные синтетические):

| Логин / пароль | Роль | Кто это |
|---|---|---|
| `admin / admin` | admin | администратор |
| `curator / curator` | curator | Анна Сергеевна — Scratch и Minecraft |
| `curator2 / curator2` | curator | Павел Игоревич — Python |
| `student / student` | student | Маша Королёва — Scratch + Minecraft, идёт в графике |
| `liza`, `artem`, `sofia`, `timur`, `ivan`, `nikita`, `vera` (пароль `student`) | student | разные ситуации: застряла на шаге, не заходит неделю, опережает группу и т. д. |
...
| DELETE | `/api/admin/courses/{id}` | в архив |
